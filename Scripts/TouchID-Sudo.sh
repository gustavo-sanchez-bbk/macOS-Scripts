...
# This script allows users to use sudo with touch ID rather than typing a password. 

# In Ventura and below, users will need to run the script again after each macOS update.
# Since Sonoma, Touch ID for sudo will persist through updates. This is because it creates a sudo_local file separately 
# Gustavo Sanchez 2023
...
#Lets future proof this for the impending release of Sonoma
macos14above() {
	# Reading 3rd line
...
if [ "$macos" -ge "14" ]; then
	echo "macOS is 14 Sonoma or above"
	macos14above
elif [ "$macos" -lt "14" ]; then
	echo "macOS is 13 Ventura or below"
	macos13below
fi
